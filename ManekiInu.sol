...
contract ManekiInu is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string constant _name = "MANEKI INU";
    ...
    uint256 public sellTotalFees = 17;
     // transfer fees
    uint256 public transferDividendRewardsFee = 0;
    ...
    uint256 public transferTotalFees = 0;

    address public marketingFeeReceiver = 0x56ffc0c5DdeE9E7550526b60eBBB9bA43af5Be6D;
    address public devFeeReceiver = 0x612D85c1556Fd3971db7EfA2c0452C9C099d5740;
    ...
    uint256 public swapThreshold = (_totalSupply * 10) / 100000; // 0.01% of supply
    bool inSwap;
    modifier swapping() {
        inSwap = true;
        _;
    ...
    function _transferFrom(
        ...
        if (isGenerating) {
            require(isWhitelisted[sender] || isWhitelisted[recipient], "Generating in progress");
    ...
    function takeFee(
        ...
        if (to == pair) {
            ...
            feeAmount = amount.mul(transferTotalFees).div(100);

            if (transferBurnFee > 0) {
                burnFee = feeAmount.mul(transferBurnFee).div(transferTotalFees);
                _balances[DEAD] = _balances[DEAD].add(burnFee);
                emit Transfer(sender, DEAD, burnFee);
            }
        }
    ...
    function updateTransferFees(
        ...
        transferDividendRewardsFee = reward;
        transferMarketingFee = marketing;
        transferLiquidityFee = liquidity;
        transferDevFee = dev;
        transferBurnFee = burn;
        transferTotalFees = reward.add(marketing).add(liquidity).add(dev).add(burn);
        require(transferTotalFees <= 25, "Total Fee must be less than 25%");
    }
    ...
    function toggleGenerating(bool value) external onlyOwner {
        isGenerating = value;
    }
